import fs from 'fs';
import { Token, Pool } from '@/src/temporal/activities/flow7d/helpers/types';
...
import {
  ...
  STABLE_POOL_ABI,
} from '@/src/temporal/activities/flow7d/helpers/config/constants';
import {
  ERC20_ABI,
  TOKENS,
  CHAIN_ID,
...
import Tokens from '@/src/temporal/activities/flow7d/helpers/config/tokens';
import assert from 'assert';

export const get_token = async (tokenAddress: string, network: 'TESTNET' | 'MAINNET'): Promise<Token> => {
  const tokenConfigCopy = JSON.parse(JSON.stringify(Tokens[network]));

  let token: Token | null = null;

  for (let i = 0; i < tokenConfigCopy.length; i++) {
    if (
      BigInt(tokenConfigCopy[i].address) === BigInt(tokenAddress) &&
      tokenConfigCopy[i].chainId === CHAIN_ID[network]
    ) {
      token = tokenConfigCopy[i];
      break;
    }
  }
  ...
  assert(
    token,
    `Error: Can't find token ${tokenAddress} on network ${network}, please add it to relevant config/tokens.ts`
  );

  if (token?.address && is_native(token?.address)) token.address = TOKENS[network].weth;
...
export const get_pool = async (tokenA: Token, tokenB: Token, network: string, signer: Wallet): Promise<Pool> => {
  ...
  if (pool_address === ZERO_ADDRESS) throw 'Error: Pool does not exist yet.';
...
export const get_stable_pool = async (tokenA: Token, tokenB: Token, network: string, signer: Wallet): Promise<Pool> => {
  const Stable_pool_factory: Contract = new Contract(STABLE_POOL_FACTORY[network], STABLE_POOL_FACTORY_ABI, signer);
  const pool_address: string = await Stable_pool_factory.getPool(tokenA.address, tokenB.address);

  if (pool_address === ZERO_ADDRESS) throw 'Error: Stable pool does not exist yet.';

  const Pool = new Contract(pool_address, STABLE_POOL_ABI, signer);
  const reserves = await Pool.getReserves();
...
export const is_balance = async (signer: Wallet, addressA: string, addressB: string): Promise<number> => {
  ...
  if (balanceA.string === '0.0' || balanceB.string === '0.0') return 0;
  else return 1;
};
...
export const sort_tokens = (
  ...
  const amount0 =
    token0.address === tokenA.address
      ? ethers.parseUnits(amountA ?? '0', token0.decimals)
      : ethers.parseUnits(amountB ?? '0', token0.decimals);

  const amount1 =
    token1.address === tokenA.address
      ? ethers.parseUnits(amountA ?? '0', token1.decimals)
      : ethers.parseUnits(amountB ?? '0', token1.decimals);
