...
import { AddLiquidity, Pool, Token } from '@/src/temporal/activities/flow7b/helpers/types/';
import { TICKER, ZERO_ADDRESS, TOKENS } from '@/src/temporal/activities/flow7b/helpers/config/constants';
...
export const get_add_liq_tx = async (
  ...
  if (max) {
    ...
    const addr: string = amountA ? addressA : addressB;
    const amount: string = amountA ? amountA : amountB!;
...
const get_max_liq = async (
  ...
  const balanceA = await get_balance(
    pool.tokenA.address === TOKENS[network].weth ? ZERO_ADDRESS : pool.tokenA.address,
    signer
  );
  const balanceB = await get_balance(
    pool.tokenB.address === TOKENS[network].weth ? ZERO_ADDRESS : pool.tokenB.address,
    signer
  );
  ...
  const balance_a: bigint = b_is_min_balance ? ethers.parseUnits(quoteA, balanceA.decimals) : balanceA.bigint;
  const balance_b: bigint = b_is_min_balance ? balanceB.bigint : ethers.parseUnits(quoteB, balanceB.decimals);
  ...
  if (is_native(addressA) || is_native(addressB))
    value = is_native(pool.tokenA.address === TOKENS[network].weth ? ZERO_ADDRESS : pool.tokenA.address)
      ? balance_a
      : balance_b;
...
const get_liq = async (
  ...
  if (amount_1 > balance_1.bigint) throw new Error(`${TICKER[token_1.address]}: Unsufficient balance.`);
  if (amount_2 > balance_2.bigint)
    throw new Error(
      `${TICKER[token_2.address]}: Unsufficient balance.\nNeeded ${ethers.formatUnits(
        amount_2,
        token_2.decimals
      )} but got ${balance_2.string}`
    );
...
export const get_min_liquidity = async (addTx: AddLiquidity, Router: Contract): Promise<bigint> => {
  ...
  const min = await Router.addLiquidity.staticCall(pool, inputs, data, minLiquidity, callback, callbackData, {
    // eslint-disable-next-line object-shorthand
    value: value,
  });
