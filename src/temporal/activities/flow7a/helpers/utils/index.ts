import fs from 'fs';
import { Token, Pool } from '@/src/temporal/activities/flow7a/helpers/types';
import { ethers, Wallet, Contract } from 'ethers';
import { CLASSIC_POOL_FACTORY_ABI, CLASSIC_POOL_ABI } from '@/src/temporal/activities/flow7a/helpers/config/constants';
import {
  ERC20_ABI,
  TOKENS,
  CHAIN_ID,
...
} from '@/src/temporal/activities/flow7a/helpers//config/constants';
import Tokens from '@/src/temporal/activities/flow7a/helpers/config/tokens';

export const get_token = async (tokenAddress: string, network: 'TESTNET' | 'MAINNET'): Promise<Token> => {
  const token = Object.values(Tokens[network]).find((token: Token) => {
  ...
  if (is_native(token.address)) token.address = TOKENS[network].weth;
